...
# Determine iOS simulator SDK path at runtime
SDK_PATH=$(xcrun --sdk iphonesimulator --show-sdk-path)
CLANGPP=$(xcrun --find clang++)
CLANG=$(xcrun --find clang)

# Write user-config.jam for cross-compilation to iOS Simulator arm64
cat > user-config.jam << EOF
using clang : ios_sim
    : "${CLANGPP}"
    : <cflags>"-arch arm64 -target arm64-apple-ios${DEPLOYMENT_TARGET}-simulator -isysroot ${SDK_PATH} -fembed-bitcode-marker"
      <cxxflags>"-arch arm64 -target arm64-apple-ios${DEPLOYMENT_TARGET}-simulator -isysroot ${SDK_PATH} -fembed-bitcode-marker -std=c++17"
      <linkflags>"-arch arm64 -target arm64-apple-ios${DEPLOYMENT_TARGET}-simulator -isysroot ${SDK_PATH}"
